...
- [Vet](https://pkg.go.dev/cmd/vet) Is the default go linter, bundled with the toolchain. If finds rather critical bugs that should be addressed right away. To execute it run `go vet ./go/...`. This tool is currently enabled in the CI, and pull request will be prevented from merging when checks are not satisfied.
...
There is no current static analysis for the evmzero. Integrating clang-tidy should not be too difficult. 
