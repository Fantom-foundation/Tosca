...
static_assert(sizeof(uint256_t) == sizeof(Word));
...
Word Word::operator+(const Word& other) const {
  auto a = std::bit_cast<uint256_t>(*this);
  auto b = std::bit_cast<uint256_t>(other);
...
Word Word::operator/(const Word& denom) const {
  ...
  auto a = std::bit_cast<uint256_t>(*this);
  auto b = std::bit_cast<uint256_t>(denom);
  return std::bit_cast<Word>(a / b);
}
...
Word Word::operator<<(const Word& shift) const {
  auto a = std::bit_cast<uint256_t>(*this);
  auto b = std::bit_cast<uint256_t>(shift);
  return std::bit_cast<Word>(a << b);
}
