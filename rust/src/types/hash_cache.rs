...
pub fn hash(data: &[u8]) -> u256 {
    ...
    if data.len() == 32 {
        ...
        return HASH_CACHE_32.get_or_insert_ref(data, || sha3(data));
        #[cfg(feature = "thread-local-cache")]
        return HASH_CACHE_32.with(|cache| cache.get_or_insert_ref(data, || sha3(data)));
    } else if data.len() == 64 {
        // SAFETY:
        // data has length 64 so it is safe to cast it to &[u8; 64].
        let data = unsafe { &*(data.as_ptr() as *const [u8; 64]) };
        #[cfg(not(feature = "thread-local-cache"))]
        return HASH_CACHE_64.get_or_insert_ref(data, || sha3(data));
        #[cfg(feature = "thread-local-cache")]
        return HASH_CACHE_64.with(|cache| cache.get_or_insert_ref(data, || sha3(data)));
    } else {
        sha3(data)
    }
