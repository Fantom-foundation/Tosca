...
const fn gen_jumptable<const STEPPABLE: bool>() -> [OpFn<STEPPABLE>; 256] {
    [
        ...
        |i| i.dup(15),
        |i| i.dup(16),
        |i| i.swap(const { NonZeroUsize::new(1).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(2).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(3).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(4).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(5).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(6).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(7).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(8).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(9).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(10).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(11).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(12).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(13).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(14).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(15).unwrap() }),
        |i| i.swap(const { NonZeroUsize::new(16).unwrap() }),
        |i| i.log::<0>(),
        |i| i.log::<1>(),
        |i| i.log::<2>(),
        |i| i.log::<3>(),
...
impl<const STEPPABLE: bool> Interpreter<'_, STEPPABLE> {
    ...
    fn run_op(&mut self, op: Opcode) -> OpResult {
        match op {
            ...
            Opcode::Swap1 => self.swap(const { NonZeroUsize::new(1).unwrap() }),
            Opcode::Swap2 => self.swap(const { NonZeroUsize::new(2).unwrap() }),
            Opcode::Swap3 => self.swap(const { NonZeroUsize::new(3).unwrap() }),
            Opcode::Swap4 => self.swap(const { NonZeroUsize::new(4).unwrap() }),
            Opcode::Swap5 => self.swap(const { NonZeroUsize::new(5).unwrap() }),
            Opcode::Swap6 => self.swap(const { NonZeroUsize::new(6).unwrap() }),
            Opcode::Swap7 => self.swap(const { NonZeroUsize::new(7).unwrap() }),
            Opcode::Swap8 => self.swap(const { NonZeroUsize::new(8).unwrap() }),
            Opcode::Swap9 => self.swap(const { NonZeroUsize::new(9).unwrap() }),
            Opcode::Swap10 => self.swap(const { NonZeroUsize::new(10).unwrap() }),
            Opcode::Swap11 => self.swap(const { NonZeroUsize::new(11).unwrap() }),
            Opcode::Swap12 => self.swap(const { NonZeroUsize::new(12).unwrap() }),
            Opcode::Swap13 => self.swap(const { NonZeroUsize::new(13).unwrap() }),
            Opcode::Swap14 => self.swap(const { NonZeroUsize::new(14).unwrap() }),
            Opcode::Swap15 => self.swap(const { NonZeroUsize::new(15).unwrap() }),
            Opcode::Swap16 => self.swap(const { NonZeroUsize::new(16).unwrap() }),
    ...
    fn swap(&mut self, nth: NonZeroUsize) -> OpResult {
        self.gas_left.consume(3)?;
