...
pub fn run<'a>(
    ...
    loop {
        ...
        match op {
            ...
            Opcode::Add => {
                consume_gas(&mut gas_left, 3)?;
                let [value1, value2] = stack.pop()?;
                stack.push(value1 + value2)?;
                code_state.next();
            }
            Opcode::Mul => {
                consume_gas(&mut gas_left, 5)?;
            ...
            Opcode::Sha3 => {
                ...
                consume_gas(&mut gas_left, 6 * word_size(len)?)?; // * does not overflow

                let data = memory.get_mut_slice(offset, len, &mut gas_left)?;
            ...
            Opcode::CallDataLoad => {
                ...
                let call_data = message.input().map(Vec::as_slice).unwrap_or_default();
            ...
            Opcode::CallDataSize => {
                consume_gas(&mut gas_left, 2)?;
                let call_data_len = message.input().map(Vec::len).unwrap_or_default();
            ...
            Opcode::CallDataCopy => {
                ...
                if len != u256::ZERO {
                    ...
                    let src = message.input().map(Vec::as_slice).unwrap_or_default();
                    let src = src.get_within_bounds(offset, len);
                    let dest = memory.get_mut_slice(dest_offset, len, &mut gas_left)?;
            ...
            Opcode::CodeCopy => {
                ...
                if len != u256::ZERO {
                    ...
                    let src = code_state.get_within_bounds(offset, len);
                    let dest = memory.get_mut_slice(dest_offset, len, &mut gas_left)?;
                    dest.copy_padded(src, &mut gas_left)?;
                }
            ...
            Opcode::ReturnDataCopy => {
                ...
                let src = last_call_return_data.as_deref().unwrap_or_default();
                let (offset, offset_overflow) = offset.into_u64_with_overflow();
                let (len, len_overflow) = len.into_u64_with_overflow();
            ...
            Opcode::BlobHash => {
                ...
                let hashes = ExecutionTxContext::from(context.get_tx_context()).blob_hashes;
            ...
            Opcode::MStore => {
                ...
                let dest = memory.get_mut_slice(offset, 32, &mut gas_left)?;
            ...
            Opcode::MStore8 => {
                ...
                let dest = memory.get_mut_byte(offset, &mut gas_left)?;
                *dest = value[31];
                code_state.next();
            }
            Opcode::SLoad => {
                if revision < Revision::EVMC_BERLIN {
                    consume_gas(&mut gas_left, 800)?;
                }
                let [key] = stack.pop()?;
                let key = key.into();
                let addr = message.recipient();
                if revision >= Revision::EVMC_BERLIN {
                    if context.access_storage(addr, &key) == AccessStatus::EVMC_ACCESS_COLD {
                        consume_gas(&mut gas_left, 2100)?;
                    } else {
                        consume_gas(&mut gas_left, 100)?;
                    }
                }
...
fn log<const N: usize>(
    ...
    let data = memory.get_mut_slice(offset, len, gas_left)?;
    // SAFETY:
    ...
    context.emit_log(message.recipient(), data, topics);
    code_state.next();
    Ok(())
}
...
fn create<const CREATE2: bool>(
    ...
    if revision >= Revision::EVMC_SHANGHAI {
        ...
        consume_gas(gas_left, init_code_cost)?;
    }
    if CREATE2 {
        let hash_cost = 6 * init_code_word_size; // does not overflow
        consume_gas(gas_left, hash_cost)?;
    }

    let init_code = memory.get_mut_slice(offset, len, gas_left)?;
    ...
    consume_gas(gas_left, gas_limit)?;
...
fn call<const CODE: bool>(
    ...
    let _dest = memory.get_mut_slice(ret_offset, ret_len, gas_left)?;
    let input = memory.get_mut_slice(args_offset, args_len, gas_left)?;
    ...
    *last_call_return_data = result.output().map(ToOwned::to_owned);
    let dest = memory.get_mut_slice(ret_offset, ret_len, gas_left)?;
    ...
    *gas_left += result.gas_left() as u64;
    consume_gas(gas_left, endowment)?;
    consume_gas(gas_left, stipend)?;
    *gas_refund += result.gas_refund();
