...
func ConvertGethToCtState(geth *gethInterpreter, state *vm.GethState) (*st.State, error) {
	...
	ctState.ReturnData = make([]byte, len(state.Result))
	copy(ctState.ReturnData, state.Result)
...
func ConvertCtStateToGeth(state *st.State) (*gethInterpreter, *vm.GethState, error) {
	...
	interpreterState.Result = make([]byte, len(state.ReturnData))
	copy(interpreterState.Result, state.ReturnData)
