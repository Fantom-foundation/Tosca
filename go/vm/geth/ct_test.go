...
func TestConvertToGeth_ReturnData(t *testing.T) {
	state := getEmptyState()
	state.ReturnData = make([]byte, 1)
	state.ReturnData[0]++
	...
	if !slices.Equal(gethState.Result, state.ReturnData) {
		t.Error("unexpected return data in geth state")
	}
}
...
func TestConvertToCt_ReturnData(t *testing.T) {
	interpreter, gethState := getEmptyGeth(ct.R07_Istanbul)
	gethState.Result = make([]byte, 1)
	gethState.Result[0]++
	...
	if !slices.Equal(gethState.Result, state.ReturnData) {
		t.Error("unexpected return data in ct state")
	}
}
