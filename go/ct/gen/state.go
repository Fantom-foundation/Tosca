...
// BindPc adds a constraint on the State's program counter to match the given variable.
...
// AddCodeOperation add a constraint to ensure the existence of an
// operation in the generated code at a variable position.
func (g *StateGenerator) AddCodeOperation(v Variable, op st.OpCode) {
...
func (g *StateGenerator) BindStackValue(pos int, variable Variable) {
	g.stackGen.BindValue(pos, variable)
}
...
func (g *StateGenerator) Generate(rnd *rand.Rand) (*st.State, error) {
	...
	fmt.Printf("Code constraints: %v\n", g.codeGen)
	fmt.Printf("assignment after code generation: %v\n", assignment)
	...
	for _, cur := range g.pcVariableConstraints {
		...
		if !found {
			return nil, fmt.Errorf("internal error: variable %v not bound to value", cur)
		}
...
func (g *StateGenerator) Restore(other *StateGenerator) {
	if g != other {
		...
		g.pcConstantConstraints = slices.Clone(g.pcConstantConstraints)
		g.pcVariableConstraints = slices.Clone(g.pcVariableConstraints)
