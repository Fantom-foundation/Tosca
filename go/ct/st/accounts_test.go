...
func TestAccounts_Clone(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"add-code": {func(accounts *Accounts) {
			accounts.SetCode(b, NewBytes([]byte{byte(tosca.ADD), byte(tosca.PUSH1), 5, byte(tosca.PUSH2)}))
		}},
		"modify-code": {func(accounts *Accounts) {
			accounts.SetCode(a, NewBytes([]byte{byte(tosca.SUB), byte(tosca.BALANCE), 5, byte(tosca.SHA3)}))
		}},
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			b1.SetCode(a, NewBytes([]byte{byte(tosca.SUB), byte(tosca.SWAP1), 5, byte(tosca.PUSH2)}))
			b1.MarkWarm(a)
			b2 := b1.Clone()
			if !b1.Eq(b2) {
...
func TestAccounts_Diff(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			a1.SetCode(a, NewBytes([]byte{byte(tosca.SUB), byte(tosca.SWAP1), 5, byte(tosca.PUSH2)}))
			a1.MarkWarm(a)
			a2 := a1.Clone()
			diff := a1.Diff(a2)
			if len(diff) != 0 {
...
func accountInit(a tosca.Address) *Accounts {
	...
	ab.SetCode(a, NewBytes([]byte{byte(tosca.SUB), byte(tosca.SWAP1), 5, byte(tosca.PUSH2)}))
	ab.SetWarm(a)
	acc := ab.Build()
	return acc
}
...
func BenchmarkAccountCloneModifyCode(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		b2 := b1.Clone()
		b2.SetCode(a, NewBytes([]byte{byte(tosca.ADD), byte(tosca.PUSH1), 5, byte(tosca.PUSH2)}))
	}
}
