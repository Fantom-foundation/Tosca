...
func TestSerialization_ImportState(t *testing.T) {
	...
	if !s.Eq(s2) {
		diffs := s.Diff(s2)
		t.Error("tosca.INVALID deserialization, differences found:")
...
func TestSerialization_ImportStateInvalidFile(t *testing.T) {
	...
	err := os.WriteFile(testFilePath, []byte("tosca.INVALID json syntax"), 0644)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSerialization_NewStateSerializableIsIndependent(t *testing.T) {
	...
	serializableState.Code.ToBytes()[0] = byte(tosca.INVALID)
	...
	serializableState.Accounts.Code[tosca.Address{0x01}] = NewBytes([]byte{byte(tosca.INVALID)})
	...
	ok := s.Status == Running &&
		...
		s.GasRefund == 63 &&
		s.Code.Length() == 5 &&
		s.Code.code[0] == byte(tosca.PUSH2) &&
		s.Stack.Size() == 1 &&
		...
		s.Accounts.GetCode(tosca.Address{0x01}) == NewBytes([]byte{byte(tosca.PUSH1), byte(6)}) &&
...
func TestSerialization_DeserializedStateIsIndependent(t *testing.T) {
	...
	deserializedState.Code.code[0] = byte(tosca.INVALID)
	...
	deserializedState.Accounts.SetCode(tosca.Address{0x01}, NewBytes([]byte{byte(tosca.INVALID)}))
	...
	ok := s.Status == Running &&
		...
		s.GasRefund == 63 &&
		s.Code.Length() == 5 &&
		s.Code.ToBytes()[0] == byte(tosca.PUSH2) &&
		len(s.Stack) == 1 &&
		s.Stack[0].Uint64() == 42 &&
		s.Memory.Length() == 64 &&
		...
		s.Accounts.Code[tosca.Address{0x01}] == NewBytes([]byte{byte(tosca.PUSH1), byte(6)}) &&
...
func TestSerialization_IncompleteSerializedData(t *testing.T) {
	...
	if want, got := Running, state.Status; want != got {
		t.Errorf("tosca.INVALID deserialization of Status, want: %v, got: %v", want, got)
	}
	if want, got := tosca.R10_London, state.Revision; want != got {
		t.Errorf("tosca.INVALID deserialization of Revision, want: %v, got: %v", want, got)
	}
	if want, got := uint16(3), state.Pc; want != got {
		t.Errorf("tosca.INVALID deserialization of Pc, want: %v, got: %v", want, got)
	}
}
