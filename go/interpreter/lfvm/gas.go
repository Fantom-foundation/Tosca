...
const (
	...
	ColdSloadCostEIP2929         = 2100 // Cost of cold SLOAD after EIP 2929
	ColdAccountAccessCostEIP2929 = 2600 // Cost of cold account access after EIP 2929
	...
	SelfdestructGasEIP150             = 5000  // Gas cost of SELFDESTRUCT post EIP-150
	SelfdestructRefundGas             = 24000 // Refunded following a selfdestruct operation.
	SloadGasEIP2200                   = 800   // Cost of SLOAD after EIP 2200 (part of Istanbul)
	SstoreClearsScheduleRefundEIP2200 = 15000 // Once per SSTORE operation for clearing an originally existing storage slot
	...
	SstoreResetGasEIP2200      = 5000  // Once per SSTORE operation from clean non-zero to something else
	SstoreSentryGasEIP2200     = 2300  // Minimum gas required to be present for an SSTORE call, not consumed
	SstoreSetGasEIP2200        = 20000 // Once per SSTORE operation from clean zero to non-zero
	WarmStorageReadCostEIP2929 = 100   // Cost of reading warm storage after EIP 2929
...
func gasSStoreEIP2200(c *context) (tosca.Gas, error) {
	...
	if c.gas <= tosca.Gas(SstoreSentryGasEIP2200) {
		c.status = OUT_OF_GAS
	...
	if current == value { // noop (1)
		return tosca.Gas(SloadGasEIP2200), nil
	}
	...
	if original == current {
		if original == zero { // create slot (2.1.1)
			return tosca.Gas(SstoreSetGasEIP2200), nil
		}
		if value == zero { // delete slot (2.1.2b)
			c.refund += tosca.Gas(SstoreClearsScheduleRefundEIP2200)
		}
		return tosca.Gas(SstoreResetGasEIP2200), nil // write existing slot (2.1.2)
	}
	if original != zero {
	...
	if original == value {
		if original == zero { // reset to original inexistent slot (2.2.2.1)
			c.refund += tosca.Gas(SstoreSetGasEIP2200 - SloadGasEIP2200)
		} else { // reset to original existing slot (2.2.2.2)
			c.refund += tosca.Gas(SstoreResetGasEIP2200 - SloadGasEIP2200)
		}
	}
	return tosca.Gas(SloadGasEIP2200), nil // dirty update (2.2)
}

func gasSStoreEIP2929(c *context) (tosca.Gas, error) {

	clearingRefund := tosca.Gas(SstoreClearsScheduleRefundEIP2200)
	if c.isLondon() {
		clearingRefund = tosca.Gas(SstoreClearsScheduleRefundEIP3529)
	}
	...
	if addrPresent, slotPresent := c.context.IsSlotInAccessList(c.params.Recipient, slot); !slotPresent {
		...
		cost = tosca.Gas(ColdSloadCostEIP2929)
	...
	if current == value { // noop (1)
		return cost + tosca.Gas(WarmStorageReadCostEIP2929), nil // SLOAD_GAS
	}
	//lint:ignore SA1019 deprecated functions to be migrated in #616
	original := c.context.GetCommittedStorage(c.params.Recipient, slot)
	if original == current {
		if original == zero { // create slot (2.1.1)
			return cost + tosca.Gas(SstoreSetGasEIP2200), nil
		}
		if value == zero { // delete slot (2.1.2b)
			c.refund += clearingRefund
		}
		return cost + tosca.Gas(SstoreResetGasEIP2200-ColdSloadCostEIP2929), nil // write existing slot (2.1.2)
	}
	if original != zero {
		if current == zero { // recreate slot (2.2.1.1)
			c.refund -= clearingRefund
	...
	if original == value {
		if original == zero { // reset to original inexistent slot (2.2.2.1)
			c.refund += tosca.Gas(SstoreSetGasEIP2200 - WarmStorageReadCostEIP2929)
		} else { // reset to original existing slot (2.2.2.2)
			c.refund += tosca.Gas((SstoreResetGasEIP2200 - ColdSloadCostEIP2929) - WarmStorageReadCostEIP2929)
		}
	}
	return cost + tosca.Gas(WarmStorageReadCostEIP2929), nil // dirty update (2.2)
}
...
func gasEip2929AccountCheck(c *context, address tosca.Address) error {
	if c.isBerlin() {
		...
		if !c.context.IsAddressInAccessList(address) {
			if !c.UseGas(tosca.Gas(ColdAccountAccessCostEIP2929 - WarmStorageReadCostEIP2929)) {
				return errOutOfGas
			}
...
func addressInAccessList(c *context) (warmAccess bool, coldCost tosca.Gas, err error) {
	...
	if c.isBerlin() {
		...
		coldCost = tosca.Gas(ColdAccountAccessCostEIP2929 - WarmStorageReadCostEIP2929)
		if !warmAccess {
...
func gasSelfdestruct(c *context) tosca.Gas {
	gas := tosca.Gas(SelfdestructGasEIP150)
	...
	if !c.context.AccountExists(address) && c.context.GetBalance(c.params.Recipient) != (tosca.Value{}) {
		gas += tosca.Gas(CreateBySelfdestructGas)
	}
	//lint:ignore SA1019 deprecated functions to be migrated in #616
	if !c.context.HasSelfDestructed(c.params.Recipient) {
		c.refund += tosca.Gas(SelfdestructRefundGas)
	}
	return gas
}
...
func gasSelfdestructEIP2929(c *context) tosca.Gas {
	...
	if !c.context.IsAddressInAccessList(address) {
		// If the caller cannot afford the cost, this change will be rolled back
		c.context.AccessAccount(address)
		gas = tosca.Gas(ColdAccountAccessCostEIP2929)
	}
