...
tosca-go: tosca-cpp
	...
	cmake -Bbuild -DCMAKE_BUILD_TYPE=Debug -DCMAKE_SHARED_LIBRARY_SUFFIX_CXX=.so ; \
...
tosca-cpp:
	cd cpp ; \
	cmake -Bbuild -DCMAKE_BUILD_TYPE="Debug" -DCMAKE_SHARED_LIBRARY_SUFFIX_CXX=.so -DTOSCA_ASSERT="ON" -DTOSCA_ASAN="OFF"; \
	cmake --build build --parallel
	# cd cpp ; \
...
test: TOSCA_CPP_BUILD = Debug
test: test-go test-cpp

test-go: TOSCA_CPP_BUILD = Debug
test-go: tosca-go
	@go test ./...

test-cpp: TOSCA_CPP_BUILD = Debug
test-cpp: tosca-cpp
	@cd cpp/build ; \
